...
convertScatterPlot <- function(app, chart_obj, chr) {
  ...
  for (ms in measurements) {
    ...
    track_list[[length(track_list)+1]] <<- DataTrack(gr_chr, groups=colnames(mcols(gr_chr)), type=c("p"), chromosome=chr, name=name)
  }
}
