...
convertHeatmapPlot <- function(app, chart_obj, chr) {
  ...
  track_list[[length(track_list)+1]] <<- DataTrack(gr_chr, groups=colnames(mcols(gr_chr)), type=c("heatmap"), chromosome=chr, name=name)
  }
}
