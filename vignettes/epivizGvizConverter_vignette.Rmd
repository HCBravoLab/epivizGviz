...
```{r, eval=F}
# create standalone workspac9
app <- startStandalone(Homo.sapiens, keep_seqlevels=paste0("chr", c(10,11,20)), chr="chr11", port=7341)
...
The `saveEpiviz` function can be used to save a connected Epiviz workspace to a ".rda" file for later use.  The `restartEpiviz` function is then used to replot the Epiviz workspace from that ".rda" file.

```{r, eval=F}
saveEpiviz(app=app, file="data/app.rda")
restartEpiviz(file="data/app.rda")
```
