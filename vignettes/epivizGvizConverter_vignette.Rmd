...
biocLite(c("Homo.sapiens", "GenomicFeatures", "TxDb.Hsapiens.UCSC.hg19.knownGene"))
...
app <- startStandalone(Homo.sapiens, keep_seqlevels=paste0("chr", c(10,11,20)), chr="chr11", port=7339)
...
The function will return the list of tracks that were successfully turned into Gviz plots. It will also automatically plot the new tracks once they are converted, unless the "plot_tracks" option is changed to FALSE.
...
```{r, eval=F}
app <- gvizToEpiviz(gviz_plot)
```

The workspace should end up looking like the one we began with in this vignette.

![](images/converted_epiviz_pic.PNG)

Once the Epiviz workspace is created, you can add other visualizations as you wish. The settings of the charts may also be altered. Here, we remove the LineTrack and replace it with a StackedLineTrack as an example, as well as change the color of the DMR tracks.
